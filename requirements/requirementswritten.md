...
- **Version 1**
    ...
    - **T8E-2: Inventory Browsing & Shopping Cart**
        ...
        - **T8S-4**: _Display inventory sorted by price_
            ...
            - _Effort_: 0.5 day
            - _Type_: Functional
            - After logging in, the user must be shown a list of all available inventory sorted from highest to lowest price. Sold items must not be shown in the list or in search results.
    ...
    - **T8E-3: Checkout and Payment Processing**
        ...
        - **T8S-9**: _Enter payment and shipping information_
            ...
            - _Effort_: 1 day
            - _Type_: Functional
            - The user must provide shipping address, phone number, credit card number, expiration month/year, and CVV. The user must also select one of the following shipping speeds: Overnight ($29), 3-Day ($19), or Ground (Free). All fields are required. A “Return to Shopping” option must also be available.
        ...
        - **T8S-12**: _Simulate email receipt display_
            ...
            - _Effort_: 0.25 day
            - _Type_: Functional
            - The same receipt shown after order completion must be displayed as a simulated email in the browser (actual email sending is not required). The simulated view must match the on-screen receipt exactly.

        - **T8S-18**: _Preserve cart if checkout is abandoned_
            - _Priority_: Must Have
            - _Effort_: 0.5 day
            - _Type_: Functional
            - If a user exits the checkout process without completing the order, the shopping cart must retain its contents for their current session.
...
- **Version 2**
    ...
    - **T8E-6: Search and Usability Enhancements**
        ...
        - **T8S-19**: _Search inventory by name or description_
